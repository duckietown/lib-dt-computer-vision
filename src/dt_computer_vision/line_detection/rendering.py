...
def draw_segments(image: BGRImage, detections: Dict[ColorRange, Detections]):
    ...
    Args:
        ...
        detections (`dict`): a dictionary that has keys :py:class:`ColorRange` and
        values :py:class:`Detection`
    ...
    for color_range, det in list(detections.items()):
        ...
        for line in det.lines:
            ...
            im = cv2.line(
                im, (line[0], line[1]), (line[2], line[3]), color=tuple([int(x) for x in color]),
                thickness=2
            )
    return im
...
def draw_maps(image, detections):
    ...
    Draws a set of color filter maps (the part of the images falling in a given color range)
    on an image.
